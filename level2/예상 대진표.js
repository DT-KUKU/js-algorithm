...
경쟁자로 생각하는 B번 참가자와 몇 번째 라운드에서 만나는지 return 하는 solution 함수를 완성해 주세요. 단, A번 참가자와 B번 참가자는 
서로 붙게 되기 전까지 항상 이긴다고 가정합니다.

문제 풀이
1. 1<->2 중 2번이 승리한다면 다음 라운드에는 1번이 된다.
2. 3<->4 중 3번이 승리하면 다음라운드에는 2번이 된다.
3. Math.round를 사용한다. 
...
const result = solution(20, 18, 20);
console.log(result);
